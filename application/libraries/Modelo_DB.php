...
class Modelo_DB {
    ...
    public function insertar_posicion($data) {
        $posicion = $this->CI->db->select(''
                                . ''.$this->tabla.'.posicion AS s_posicion')->from($this->tabla)
                        ->order_by(''.$this->tabla.'.posicion', 'desc')->limit(1)->get()->result();

        $data['posicion'] = ((count($posicion) > 0) ? $posicion[0]->s_posicion : 0) + 1;
        if ($this->CI->db->insert($this->tabla, $data)) {
            return TRUE;
        } else {
            return FALSE;
        }
    ...
    public function buscar($like) {
        $this->get_query();
        $this->get_like($like);
        $this->CI->db->where($this->alias . '.' . $this->tabla_oculto, '0');
        return $this->CI->db->get()->result_array();
    }
