...
class Complement {
        ...
        switch ($type) {
            case 'single':
                if (isset($_FILES[$file]['tmp_name']) &&
                    ...
                    $value = $_FILES[$file];
                    $result = $this->checked_extention($value['name'], 'jpeg|jpg|png|gif|pdf');
                    if ($result === FALSE) {
                        return FALSE;
                    } else {
                        return $value;
    ...
    public function save_file($file, $route, $size = 5) {
        $this->ci = & get_instance();
    ...
    public function save_multiple_file($file, $route, $size = 5) {
        $this->ci = & get_instance();
        $this->ci->load->library('file');
        for ($i = 0; $i < count($file['tmp_name']); $i++) {
            $archive = $file['name'][$i];
            $weight = $file['size'][$i];
            $tmp = $file['tmp_name'][$i];
            $new_name = $this->random();
            $this->ci->file->loadFile($archive, $route, $weight, $tmp, $new_name, $size);
            $data[] = $this->ci->file->uploadFile();
        }
        return $data;
    }
