...
class Formulario_helpingdent extends CI_Controller {
    ...
    public function __construct() {
        ...
        $model = array('m_usuario', 'm_formulario_helpingdent');
    ...
    public function listar() {
        ...
        $lista = $this->m_formulario_helpingdent->mostrar_activos(FALSE, FALSE, array("fh.fecha_registro" => "desc"));        
        if (!empty($lista)) {
            $i = 1;
            foreach ($lista AS $items) {
                $accion = $this->mantenimiento->accion($items['idformulariohelping'], 'ver2|eliminar2', 'formulario_web', $items['oculto']);
                    $data['lista'][] = array(
                        'id' => $items['idformulariohelping'],
                        'numero' => $i,
                        'nombre' => $items['nombre'],
                        'email' => $items['email'],
        ...
        $this->template->admin("listar_formulario_helpingdent", $data);
    }
    ...
    public function observar($id = '') {
        ...
        $tmp = $this->m_formulario_helpingdent->mostrar($where);
        if (!empty($tmp)) {
    ...
    public function accion_denegar($id = '') {
        ...
        $where = array('f.idformulario' => $id, 'f.oculto' => 0);
        $resultSet = $this->m_formulario_helpingdent->exists($where);
        if ($resultSet == 0) {
            echo $this->url_comp->direccionar(base_url() . 'admin/formulario_web/listar', TRUE);
        } else {
            $this->m_formulario_helpingdent->ocultar($id);
            echo $this->url_comp->direccionar(base_url() . 'admin/formulario_web/listar', TRUE);
        }
    }
}
